...
if (isDev) {
  ...
  const translateRowText = contentList
  ...
  // 调整内容
  area1.children[1].innerHTML =
    '<div>在这里粘贴 Renpy 待翻译的文本，目前仅支持 translate 语句，不支持 old/new 语句</div>'
  // 监听原代码区内容变化
  ...
  area1.addEventListener('input', () => {
    ...
    const translateRowText = contentList
      .map((content) => content.textContent)
      .join('\n')
    area3.children[0].textContent = translateRowText
  ...
  setInterval(() => {
    ...
    if (contentList.length !== translateList.length) {
      console.log(
        `原文有${contentList.length}句，翻译有${translateList.length}句，等待继续翻译中...`
      )
      return
    }
...
function rpy2content(rowString) {
  ...
  const parseResult = text.map((line, index) => {
    ...
    if (type === '2原文') {
      const [_, textPrefix, textContent, textSuffix] = line.match(
        typeTestList[2].regex
      )
      return {
        type,
...
function translate2code(translateList, contentList, parseResult) {
  ...
  const finalString = parseResult
    .map(({ type, source, translate, textPrefix, textSuffix }, index) => {
      if (type === '2原文') {
        lastIsSource = true
        return `\n${source}\n    ${
          textPrefix || ''
        }"${translate}"${textSuffix}\n`
      }
